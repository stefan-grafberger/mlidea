...
from mlidea.analysis._analysis_utils import find_dag_location_for_data_patch, add_new_node_after_node, \
...
from mlidea.instrumentation._dag_node import DagNode, OptimizerInfo, DagNodeDetails
...
class DataFiltering(DataPatch):
    ...
    def apply(self, dag: networkx.DiGraph, pipeline_executor):
        ...
        if is_before_slit is False:
            ...
            logger.warning("This could hint at data leakage!")
            add_new_node_after_node(dag, self.filter_operator, location)
...
class ModelPatch(PipelinePatch):
    ...
    def apply(self, dag: networkx.DiGraph, pipeline_executor):
        ...
        if len(estimator_nodes) != 1:
            raise NotImplementedError("Currently, ModelPatch only supports pipelines with exactly one estimator!")

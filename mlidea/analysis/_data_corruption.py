...
class CorruptionType(Enum):
    """
    The different corruption functions known to mlidea, but custom corruption functions can also be used
    """
...
class DataCorruption(WhatIfAnalysis):
    ...
    def __init__(self,
                 column_to_corruption: list[tuple[str, FunctionType | CorruptionType]],
                 corruption_percentages: Iterable[float | Callable] or None = None,
    ...
    def generate_plans_to_try(self, dag: networkx.DiGraph) -> Iterable[Iterable[PipelinePatch]]:
        ...
        if len(predict_operators) != 1:
            raise NotImplementedError("Currently, DataCorruption only supports pipelines with exactly one predict "
                                      "call which must be on the test set!")

...
from mlidea import OperatorType, DagNode, OperatorContext, DagNodeDetails
from mlidea.analysis._analysis_utils import find_nodes_by_type, find_train_or_test_pipeline_part_end, \
...
from mlidea.execution._patches import PipelinePatch, OperatorReplacement, OperatorRemoval
...
class OperatorImpact(WhatIfAnalysis):
    ...
    def generate_plans_to_try(self, dag: networkx.DiGraph) -> Iterable[Iterable[PipelinePatch]]:
        ...
        if len(predict_operators) != 1:
            raise NotImplementedError("Currently, DataCorruption only supports pipelines with exactly one "
                                      "predict call which must be on the test set!")
        ...
        if len(self._score_nodes_and_linenos) != len(set(self._score_nodes_and_linenos)):
            raise NotImplementedError("Currently, DataCorruption only supports pipelines where different score "
                                      "operations can be uniquely identified by the line number in the code!")
    ...
    def get_op_replacement_description(operator_to_replace):
        ...
        if operator_to_replace.operator_info.operator == OperatorType.TRANSFORMER:
            ...
            raise NotImplementedError(f"Replacing operator type {operator_to_replace.operator_info.operator.value} is "
                                      f"not supported yet!")
    ...
    def _detect_if_filters_shared_between_train_test_or_duplicated(all_nodes_to_test, dag):
        ...
        for node in all_nodes_to_test:
            ...
            if node in train_nodes and node in test_nodes:
                ...
                raise ValueError("A node must be either in the train or test part of the pipeline")

...
from mlidea.visualisation._visualisation import save_fig_to_path
...
def get_expected_dag_adult_easy(caller_filename: str, line_offset: int = 0, with_code_references=True):
    ...
    expected_data_source = DagNode(0,
                                   ...
                                   OperatorContext(OperatorType.DATA_SOURCE,
                                                   FunctionInfo('pandas.io.parsers', 'read_csv'), Comparison(dict)),
    ...
    expected_pipeline_project_two = DagNode(5,
                                            BasicCodeLocation(caller_filename, 18 + line_offset),
                                            OperatorContext(OperatorType.PROJECTION,
                                                            FunctionInfo('sklearn.compose._column_transformer',
                                                                         'ColumnTransformer'), Comparison(dict)),
    ...
    expected_pipeline_transformer_one = DagNode(6,
...
def run_and_assert_all_op_outputs_inspected(py_file_path, _, dag_png_path, custom_monkey_patching=None):
    ...
    Assert that mlidea properly lets inspections inspect all DAG nodes
    """
...
def run_scenario_and_visualize_dags(dataset, scenario, tmpdir, featurization="featurization_0",
    ...
    with patch.object(sys, 'argv', ["mlidea", dataset, "fast_loading", featurization, model]):

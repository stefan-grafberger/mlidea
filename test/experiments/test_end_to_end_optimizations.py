...
from mlidea.testing._testing_helper_utils import run_scenario_and_visualize_dags
...
def run_base_pipeline_and_visualize_dag(dataset, featurization, model, tmpdir):
    ...
    for node in nodes:
        if node.operator_info.operator == OperatorType.MISSING_OP:
            raise NotImplementedError(f"Missing Ops not supported currently! The operator: {node}")
